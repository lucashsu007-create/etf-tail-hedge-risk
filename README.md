# ETF Tail Hedge Risk

Research question:

Do near-substitute ETFs experience extreme intraday hedge-error events during liquidity stress, and are these events underestimated by rolling beta/correlation models?

First universe:
- SPY / IVV / VOO
- QQQ / QQQM

First data source:
- WRDS TAQ trades and quotes once access is approved

First empirical object:
HE_t = r_ETF1,t - beta_t r_ETF2,t

papers to use:
ETF Market Structure
Madhavan & Sobczyk (2016) — Price Dynamics and Liquidity of Exchange-Traded Funds
Box, Davis, Evans & Lynch (2021) — Intraday Arbitrage Between ETFs and Their Underlying Portfolios
Pan & Zeng (2019/2020) — ETF Arbitrage Under Liquidity Mismatch
Alexander & Barbosa (2008) — Hedging and Cross-Hedging ETFs
Ben-David, Franzoni & Moussawi (2018) — Do ETFs Increase Volatility?
Extreme Value Theory / Financial Risk
Nolde & Zhou (2021) — Extreme Value Analysis for Financial Risk Management ⭐ (Most important for Chen Zhou)
McNeil & Frey (2000) — Estimation of Tail-Related Risk Measures for Heteroscedastic Financial Time Series
Poon, Rockinger & Tawn (2004) — Extreme Value Dependence in Financial Markets
ETF + Tail Risk (Gap Analysis)
Ning (2024) — ETFs and Tail Dependence: Evidence from Chinese Stock Markets
Zhong (2026) — Reliability-Aware ETF Tail-Risk Monitoring
